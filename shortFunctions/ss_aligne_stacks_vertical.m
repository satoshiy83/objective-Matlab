% Short script to aligne image stack in vertical direction.
% Written by Satoshi Yamashita.

function result = ss_aligne_stacks_vertical(array)
% Short script to aligne images stacks in given array.
% result = ss_aligne_stacks_vertical(array)
...
for i = 1:array.count
    ...
    frame(1) = frame(1) + image.frameSize(1);
    frame(2) = max([frame(2),image.frameSize(2)]);
...
for i = 1:array.count
    image = array.objectAtIndex(i);
    range(1) = range(2) + 1;
    range(2) = range(1) - 1 + image.frameSize(1);
    range(4) = image.frameSize(2);
